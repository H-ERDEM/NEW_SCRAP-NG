...
try:
    ...
    with open("teknoloji_haberleri.csv", mode="w", encoding="utf-8", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Haber İsmi", "Kategori"])  # Başlık satırı

        # 4 sayfayı dolaş
        for page in range(1, 5):  # 1. sayfadan 4. sayfaya kadar
            url = f"https://www.milliyet.com.tr/teknoloji/?page={page}"
            driver.get(url)
            ...
            for haber in haberler:
                writer.writerow([haber.text, "Teknoloji"])

            print(f"{page}. sayfa işlendi.")

        print("Teknoloji kategorisindeki başlıklar CSV dosyasına kaydedildi.")
