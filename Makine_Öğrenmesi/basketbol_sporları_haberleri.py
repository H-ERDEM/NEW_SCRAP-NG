...
try:
    ...
    with open("basketbol_sporlari_haberleri.csv", mode="w", encoding="utf-8", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Haber İsmi", "Kategori"])  # Başlık satırı

        # 4 sayfayı dolaş
        for page in range(1, 5):  # 1. sayfadan 4. sayfaya kadar
            url = f"https://www.milliyet.com.tr/skorer/basketbol-haberleri/?page={page}"
            driver.get(url)
            ...
            for haber in haberler:
                writer.writerow([haber.text, "Basketbol_Sporları"])

            print(f"{page}. sayfa işlendi.")

        print("Basketbol Sporları kategorisindeki başlıklar CSV dosyasına kaydedildi.")
