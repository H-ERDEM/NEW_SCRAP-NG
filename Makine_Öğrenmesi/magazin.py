...
try:
    ...
    with open("magazin_haberleri.csv", mode="w", encoding="utf-8", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Haber İsmi", "Kategori"])  # Başlık satırı

        # 1. ve 2. sayfayı da ekle
        for page_num in range(1, 9):
            # Sayfayı aç
            url = f"https://www.milliyet.com.tr/magazin/?page={page_num}"
            driver.get(url)
            ...
            haberler = driver.find_elements(By.XPATH, "//div[contains(@class, 'cadde-news-item-content')]/strong")

            # Haber başlıklarını ve kategoriyi CSV dosyasına kaydet
            for haber in haberler:
                writer.writerow([haber.text, "Magazin"])
