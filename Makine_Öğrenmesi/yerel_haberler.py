...
try:
    ...
    with open("yerel_haberleri.csv", mode="w", encoding="utf-8", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Haber İsmi", "Kategori"])  # Başlık satırı

        # 5 sayfayı dolaş
        for page in range(1, 6):  # 1. sayfadan 5. sayfaya kadar
            url = f"https://www.milliyet.com.tr/yerel-haberler/?page={page}"
            driver.get(url)
            ...
            for haber in haberler:
                writer.writerow([haber.text, "Yerel"])

            print(f"{page}. sayfa işlendi.")

        print("Yerel kategorisindeki başlıklar CSV dosyasına kaydedildi.")
